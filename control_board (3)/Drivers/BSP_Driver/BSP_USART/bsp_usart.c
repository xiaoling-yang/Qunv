...
extern volatile uint32_t Target_Turns;//目标圈数
...
void ProcessData(void) {
	...
	speed|=Data_Manage_Unity.valid_data[0] ;
	speed|=Data_Manage_Unity.valid_data[1]<<8;
	
	turns|=Data_Manage_Unity.valid_data[2];
	turns|=Data_Manage_Unity.valid_data[3]<<8;
	turns|=Data_Manage_Unity.valid_data[4]<<16;
	turns|=Data_Manage_Unity.valid_data[5]<<24;
	
	dir=Data_Manage_Unity.valid_data[6];
	motor_state=Data_Manage_Unity.valid_data[7];
	
	
	
	Motor_State(motor_state);//设置电机运行状态， 启动或停止 1启动 0停止
