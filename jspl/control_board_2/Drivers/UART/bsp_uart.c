...
extern read_data_t current_motor_stat;//通过读到的数据获取当前电机的状态的
extern uint16_t Target_Turns;
extern uint8_t receive_byte;
extern uint8_t rx_buffer1[];
extern uint8_t rx_buffer2[];
extern osSemaphoreId_t uartBinarySem03Handle;//串口数据处理二值信号量
extern osSemaphoreId_t calculateBinarySem04Handle;//计算二值信号量
uint8_t Read_data_response_flag=1;//读数据响应标志位
...
void update_lcd_ui(char *addr,uint16_t value)
{
	    uint8_t addr_len = strlen(addr) + 1;
    char *temp_str = malloc(addr_len + 12);  // 12 是固定字符串 "=%d\xdd\xff\xff" 的长度
    ...
    // 构建要发送的数据
    int len = sprintf(temp_str, "%s=%d\xff\xff\xff", addr, value);
    
    // 发送数据
    HAL_UART_Transmit(&huart1, (uint8_t*)temp_str, len, 100);
...
if (huart==&huart2) {
    ...
    motor_data_management.crc16_check=crc16_temp;//校验值
    motor_data_management.station_number= rcceive_data_buf[0];//站号
    motor_data_management.function_code=rcceive_data_buf[1];//功能码
    if (crc16_temp== crc16_modbus(rcceive_data_buf,data_len-2)) {
        ...
        }else if (data_len==7) {//这里可以不用管
            motor_data_management.data_len=rcceive_data_buf[2];
            motor_data_management.data[0]=rcceive_data_buf[3];
            motor_data_management.data[1]=rcceive_data_buf[4];
        }
        }
        ...
        motor_state=rcceive_data_buf[7];
			current_motor_stat.motor_dir=dir;//直接通过下发的命令获取电机方向，可能会不准
        if (motor_state) {
                motor_set_speed(speed);//设置电机转速,0 - 10000
					      //HAL_Delay(500);
					osDelay(500);//电机那边处理需要时间
                motor_start_dir(dir);//设置电机运转方向 1 正 0反
                Target_Turns=turns;//设定目标 圈数
					
					
					osSemaphoreRelease(calculateBinarySem04Handle);//开始计算圈数
        }else {//停止运行
						
          motor_stop();
					osSemaphoreAcquire(calculateBinarySem04Handle,portMAX_DELAY);//停止计算圈数
        }
...
void UART1_IdleCallback() {//lcd 屏的数据暂存在这个结构体里
   ...
   // memset(&motor_data_management,0,sizeof(motor_data_management));
    HAL_UART_Receive_DMA(&huart1,rx_buffer1,RX_BUFFER_SIZE);//启用DMA接收
		//osSemaphoreRelease(uartBinarySem03Handle);//释放uart串口处理信号量信号量
